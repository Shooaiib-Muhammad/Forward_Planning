...
class Linewise extends CI_Controller
{
	...
	{
 		$data['CallMPno'] = $this->ID->GetAllMPNos();
		$data['GetLines'] = $this->ID->getAllLines();
...
$data = $this->ID->updateRecord($RIDValue,$Status,$IssueDte);
  ...
  $data = $this->ID->insertion($Qty,$MPNo,$lineID,$plan_date);
        return $this->output
        ->set_content_type('application/json')
        ->set_status_header(200)
        ->set_output(json_encode($data));
...
public function json_by_machine($Type){
        ...
        $data = $this->ID->MPNoinformation($Type);
...
/* 		$location = $this->input->post('locationName');
  ...
  public function getDataMpData($plan_date){
   					// print_r($data['received']);
					// die;
     $data['getMPData']=$this->ID->getDataMpData($plan_date);
